...
- **`plugins` overwrites the default set rather than extending it** — `typescript`, `import`, `promise`, `node` cover the TS/Node project surface. The `eslint`, `oxc`, and `unicorn` defaults stay off precisely because the array overwrites them; list one to bring it back.
